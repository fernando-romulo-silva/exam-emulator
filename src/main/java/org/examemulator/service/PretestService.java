...
import org.examemulator.domain.pretest.PretestOption;
...
public class PretestService {
	...
	final var pretestGroup = new PretestGroup(dir);

	final var pretest = new Pretest(extractedExamName(dir), pretestGroup);
    ...
    private PretestQuestion loadQuestion(final String questionName, final String data) {
	...
	return new PretestQuestion.Builder().with($ -> {
	    $.name = questionName;
    ...
    private List<PretestOption> readOptions(final String data, final List<String> correctOptions) {
	...
	return answers.entrySet()//
			.stream() //
			.map(e -> new PretestOption(e.getKey(), e.getValue(), correctOptions.contains(e.getKey()))) //
			.toList();
    }
